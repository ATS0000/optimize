...
wb = xlsxwriter.Workbook('NN+2optM100.xlsx')
ws = wb.add_worksheet()
...
for N in range(3, 100, 9):
    ...
    for a in range(M):
        ...
        '''
        # 適当に初期解を生成する↓
        first_order = list(np.random.permutation(N)) #N個の順番を並び替えたリスト。これが初期順序になる
        ...
        #print('訪問順序 = {}'.format(first_order)) #print(np.array(first_order))と同じ
        distance_value_before = total_distance
        # 適当に初期解を生成する↑
        '''
        ...
        distance_value_after = total_distance
        #print('訪問順序 = {}'.format(improved)) #print(np.array(test_order))と同じ
        #print(f'計算時間：{finishtime - starttime}')
        ws.write(a, t, distance_value_before)
        ws.write(a, s, distance_value_after)

        #plt.scatter(a, distance_value_before, c="red")
        #plt.scatter(a, distance_value_after, c="blue")
