...
for j in range(vn):
    ...
    if j != 0:
        k += math.ceil(cp/vn)-1
    p = h*np.random.rand()*np.random.choice([-1,1])
    q = h*np.random.rand()*np.random.choice([-1,1])
...
book = openpyxl.load_workbook('city_position2.xlsx')
# シートを取得
sheet = book['Sheet1']
for i in range(cp):
