...
vn = 5 #vehicle number
cp = 100 #collection place+1(include depot)
cp_list = [i for i in range(1,cp)]
MAP_SIZE = 10

print(math.ceil(MAP_SIZE/vn))
...
for j in range(vn):
    ...
    if j != 0:
        k += math.ceil(cp/vn)-1
    p = 10*MAP_SIZE*np.random.rand()
    q = 10*MAP_SIZE*np.random.rand()
...
result1 = linkage(d_matrix_pd.iloc[:, :], metric = 'cosine', method = 'ward')

print(result1)
#print(get_cluster_by_number(result1, 5))

threshold = 1000 * np.max(result1[:, 2])

plt.figure(figsize=(8, 8))
dendrogram(result1, orientation='right', labels=list(d_matrix_pd.index), color_threshold=threshold)
plt.title("Dedrogram")
...
# クラスタリング結果の値を取得
clustered = fcluster(result1, 0.8, criterion='distance')
# クラスタリング結果を比較
print(clustered)
for i in range(cp):
    order[clustered[i]-1].append(i)
