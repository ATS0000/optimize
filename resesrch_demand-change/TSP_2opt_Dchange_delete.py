...
for i in range(N):
    ax.scatter(x[i], y[i], c='red') #入力の順番に注意
...
ims = []
#図を作ってプロットを出す関数
def visualize_visit_order(order, distance_matrix, improve_with_2opt, city_xy):
    """Visualize traveling path for given visit order"""

    print(order)
    for i in range(len(order)):

        if i == 3 or i % 5 == 1: #この条件の時、需要が変わる（キャンセルされる）
            changing_demand(order, i)
...
def changing_demand(order, i):
    order.pop(np.random.randint(i+1, len(order)))
...
visualize_visit_order(improved, distance_matrix, improve_with_2opt, city_xy)
