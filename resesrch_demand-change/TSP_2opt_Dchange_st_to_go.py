...
#animation付き。
#需要（キャンセル、追加予約）が変われば、それに対応する。
#需要が変わったら、それまでの配送先は保存して、残りの配送先のみに対して再度2-opt。
...
ims = []
t_delete = 7 #demand_deleteの回数が少なくなる
t_add = 10 #demand_addの回数が少なくなる
#図を作ってプロットを出す関数
def visualize_visit_order(order, distance_matrix, improve_with_2opt, city_xy, reserve_list):
    ...
    for i in range(len(order) - math.ceil(N/t_delete) + math.ceil(N/t_add) + 1):

        if i % t_delete == 0: #この条件の時、需要が変わる（キャンセルされる）
            p = Delete_demand(order, i)
            reCalculation(order, distance_matrix, improve_with_2opt, city_xy, i, len(order))
            #im1 = ax.scatter(x[order[p]], y[order[p]], s=200, c='black', marker='x')
            print('Delete--->')
            print(order)
            d % 1

        if i % t_add == 0: #この条件の時、需要が変わる（予約が増える）
            q = Add_demand(order, reserve_list, i)
            im1 = ax.scatter(x[order[q]], y[order[q]], s=200, c='red', marker='*')
            reCalculation(order, distance_matrix, improve_with_2opt, city_xy, i, len(order))
            print('Add--->')
            print(order)
            a += 1
...
def improve_with_2opt(visit_order, distance_matrix, s, g):
...
def local_search(visit_order, distance_matrix, improve_func, city_xy, s, g): #improve_func = improve_with_2opt
    """Main procedure of local search"""
    cost_total = calculate_total_distance(visit_order, distance_matrix)

    while True:

        improved = improve_func(visit_order, distance_matrix, s, g)
...
def Delete_demand(order, i):
    p = np.random.randint(i+1, len(order)) #delete point
    order.pop(p) #delete_demand
    return p

def Add_demand(order, reserve_list, i):
    q = np.random.randint(i+1, len(order)) #add point
    order.insert(q, reserve_list[np.random.randint(len(reserve_list))])
    return q


def reCalculation(order, distance_matrix, improve_with_2opt, city_xy, s, g):
    order = local_search(order, distance_matrix, improve_with_2opt, city_xy, s, g)
...
# 近傍を計算
improved = local_search(test_order, distance_matrix, improve_with_2opt, city_xy, s, g)
